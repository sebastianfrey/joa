...
public class BboxParam {
  ...
  public List<Double> parse(String input) throws Exception {
    ...
    try {
      return Stream.of(input.trim().split(SEPARATOR)).map((part) -> Double.valueOf(part)).toList();
  ...
  public Bbox get() {
    ...
    return new Bbox().minX(minX).minY(minY).minZ(minZ).maxX(maxX).maxY(maxY).maxZ(maxZ);
  }
}
